...
from scripts.ssh.shell_params import SCRIPT_PARAMS, PARAM_LABELS
...
class SSHScriptShell:
    ...
    def _banner(self):
        art = pyfiglet.figlet_format("SSH", font="slant")
        console.print(f"[bold {COLOR}]{art}[/bold {COLOR}]", end="")
        console.print(
            Panel(
                f"[dim]Consola interactiva de scripts [bold {COLOR}]SSH[/bold {COLOR}] — Lobera[/dim]\n"
    ...
    def _prompt(self):
        if self._script_name:
            return (
                f"[bold {COLOR}]ssh-shell[/bold {COLOR}]"
                f"([bold white]{self._script_name}[/bold white]) > "
            )
        return f"[bold {COLOR}]ssh-shell[/bold {COLOR}] > "
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                console.print("\n[dim]Saliendo de la consola SSH.[/dim]")
                break
            if not line:
                continue
            ...
            args  = parts[1:]
            if cmd in ("exit", "quit"):
                console.print("[dim]Saliendo de la consola SSH.[/dim]")
                break
    ...
    def _cmd_help(self):
        ...
        for cmd, desc in [
            ("list",             "Lista los scripts SSH agrupados por familia"),
    ...
    def _cmd_list(self):
        ...
        tree = Tree(f"[bold {COLOR}]SSH[/bold {COLOR}]")
    ...
    def _cmd_load(self, name):
        ...
        _PERSISTENT  = {"target", "user", "password", "port", "timeout"}
    ...
    def _cmd_load_fam(self, family):
        ...
        console.print(Panel(
            ...
            f"  [dim]— módulo [bold {COLOR}]SSH[/bold {COLOR}][/dim]\n\n"
        ...
        creds  = Creds(user=shared_params.get("user",""), password=shared_params.get("password",""),
                       domain="", hash=None)
    ...
    def _print_script_card(self):
        ...
        console.print(Panel(
            ...
            f"  [dim]— [bold {COLOR}]SSH[/bold {COLOR}] / {fam}[/dim]\n\n{meta['description']}",
        ...
        for group in meta.get("mutually_exclusive",[]):
            console.print(f"  [yellow]⚠[/yellow]  [dim]{' y '.join(group)} son mutuamente excluyentes.[/dim]")
    ...
    def _cmd_set(self, key, value):
        ...
        if key in ("delay", "threshold"):
            try: value = float(value)
            except ValueError: console.print(f"[red]{key} debe ser decimal.[/red]"); return
    ...
    def _cmd_run(self):
        ...
        creds  = Creds(user=self._params.get("user",""), password=self._params.get("password",""),
                       domain="", hash=None)
    ...
    def _find_script_path(self, name):
        scripts_dir = self._root / "scripts" / "ssh"
        for py in sorted(scripts_dir.rglob("*.py")):
