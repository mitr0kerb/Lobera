...
from scripts.ssl.shell_params import SCRIPT_PARAMS, PARAM_LABELS
...
class SSLScriptShell:
    ...
    def _banner(self):
        art = pyfiglet.figlet_format("SSL", font="slant")
        console.print(f"[bold {COLOR}]{art}[/bold {COLOR}]", end="")
        console.print(
            Panel(
                f"[dim]Consola interactiva de scripts [bold {COLOR}]SSL[/bold {COLOR}] — Lobera[/dim]\n"
    ...
    def _prompt(self):
        if self._script_name:
            return (
                f"[bold {COLOR}]ssl-shell[/bold {COLOR}]"
                f"([bold white]{self._script_name}[/bold white]) > "
            )
        return f"[bold {COLOR}]ssl-shell[/bold {COLOR}] > "
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                console.print("\n[dim]Saliendo de la consola SSL.[/dim]")
                break
            if not line:
                continue
            ...
            args  = parts[1:]
            if cmd in ("exit", "quit"):
                console.print("[dim]Saliendo de la consola SSL.[/dim]")
                break
    ...
    def _cmd_help(self):
        ...
        for cmd, desc in [
            ("list",             "Lista los scripts SSL agrupados por familia"),
            ("load <script>",    "Carga un script individual  (ej: load cert-info)"),
    ...
    def _cmd_list(self):
        ...
        tree = Tree(f"[bold {COLOR}]SSL[/bold {COLOR}]")
    ...
    def _cmd_load(self, name):
        ...
        _PERSISTENT  = {"target", "port", "timeout", "sni"}
    ...
    def _cmd_load_fam(self, family):
        ...
        console.print(Panel(
            ...
            f"  [dim]— módulo [bold {COLOR}]SSL[/bold {COLOR}][/dim]\n\n"
    ...
    def _print_script_card(self):
        ...
        console.print(Panel(
            ...
            f"  [dim]— [bold {COLOR}]SSL[/bold {COLOR}] / {fam}[/dim]\n\n{meta['description']}",
    ...
    def _cmd_set(self, key, value):
        ...
        if key in ("port", "timeout", "attempts"):
            try: value = int(value)
    ...
    def _find_script_path(self, name):
        scripts_dir = self._root / "scripts" / "ssl"
        for py in sorted(scripts_dir.rglob("*.py")):
