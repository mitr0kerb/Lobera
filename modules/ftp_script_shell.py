...
from scripts.ftp.shell_params import SCRIPT_PARAMS, PARAM_LABELS
...
class FTPScriptShell:
    ...
    def _banner(self):
        art = pyfiglet.figlet_format("FTP", font="slant")
        console.print(f"[bold {COLOR}]{art}[/bold {COLOR}]", end="")
        console.print(
            Panel(
                f"[dim]Consola interactiva de scripts [bold {COLOR}]FTP[/bold {COLOR}] — Lobera[/dim]\n"
    ...
    def _prompt(self):
        if self._script_name:
            return (
                f"[bold {COLOR}]ftp-shell[/bold {COLOR}]"
                f"([bold white]{self._script_name}[/bold white]) > "
            )
        return f"[bold {COLOR}]ftp-shell[/bold {COLOR}] > "
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                console.print("\n[dim]Saliendo de la consola FTP.[/dim]")
                break
            if not line:
                continue
            ...
            args  = parts[1:]
            if cmd in ("exit", "quit"):
                console.print("[dim]Saliendo de la consola FTP.[/dim]"); break
    ...
    def _cmd_help(self):
        ...
        for cmd, desc in [
            ("list",           "Lista los scripts FTP agrupados por familia"),
            ("load <script>",  "Carga un script individual  (ej: load banner-grab)"),
    ...
    def _cmd_list(self):
        ...
        tree = Tree(f"[bold {COLOR}]FTP[/bold {COLOR}]")
    ...
    def _cmd_load(self, name):
        ...
        _PERSISTENT  = {"target", "user", "password", "timeout", "port"}
    ...
    def _cmd_load_fam(self, family):
        ...
        console.print()
        console.print(Panel(
            f"[bold white]FAMILIA: {family.upper()}[/bold white]  [dim]— módulo [bold {COLOR}]FTP[/bold {COLOR}][/dim]\n\n"
        ...
        for p in sorted(all_optional):
            ...
            if val:
                shared_params[p] = val
        console.print()
        target = Target(ip=shared_params.get("target",""), domain="", timeout=int(shared_params.get("timeout",5)))
        creds  = Creds(user=shared_params.get("user",""), password=shared_params.get("password",""), domain="", hash=None)
        _base  = {"target","timeout","user","password"}
    ...
    def _print_script_card(self):
        ...
        console.print()
        console.print(Panel(
            f"[bold white]{name.upper()}[/bold white]  [dim]— [bold {COLOR}]FTP[/bold {COLOR}] / {fam}[/dim]\n\n{meta['description']}",
    ...
    def _cmd_run(self):
        ...
        if missing:
            ...
            return
        target = Target(ip=self._params.get("target",""), domain="", timeout=int(self._params.get("timeout",5)))
        creds  = Creds(user=self._params.get("user",""), password=self._params.get("password",""), domain="", hash=None)
    ...
    def _find_script_path(self, name):
        scripts_dir = self._root / "scripts" / "ftp"
        for py in sorted(scripts_dir.rglob("*.py")):
