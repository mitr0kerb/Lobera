...
class Script(BaseScript):
    ...
    protocol    = "https"
    category    = "enum"
    description = "Obtiene headers del servidor HTTPS, detecta tecnologías, versiones y WAF."

    EXAMPLES = [
        {"flag": "-t / --port", "desc": "IP y puerto HTTPS",
         "good": "https --script=banner-grab -t 10.10.10.5 --port 443",
    ...
    def run(self, **kwargs):
        ...
        interesting = [
            ...
            "x-amz-request-id","strict-transport-security",
        ]
        headers_lower = {k.lower(): v for k, v in headers.items()}
        rows = [(h, headers_lower[h]) for h in interesting if h in headers_lower]

        print_result("HTTPS", ip, "info", f"HTTP {status} — {url}")
        if rows:
        ...
        session_db.save_finding(ip, "HTTPS", "banner", f"HTTP {status} port={port}")
