# scripts/http/attack/sqli_detect.py
import urllib.request, urllib.error, urllib.parse, time, re
...
class Script(BaseScript):
    ...
    EXAMPLES = [
        {"flag": "--path / --param", "desc": "Ruta y parámetro a probar",
         "good": "http --script=sqli-detect -t 10.10.10.5 --path /search --param q",
         "bad":  "http --script=sqli-detect -t 10.10.10.5 (usa defaults / y param 'id')"},
    ]
            ...
            try:
                ...
                for pattern in SQLI_ERROR_PATTERNS:
                    if re.search(pattern, body, re.I):
                        ...
                        session_db.save_finding(ip, "HTTP", "sqli_error_based",
        ...
        for db_name, payload in TIME_PAYLOADS:
            ...
            try:
                ...
                if elapsed >= 2.5:
                    ...
                    session_db.save_finding(ip, "HTTP", "sqli_time_based",
                                            f"param={param} db={db_name} delay={elapsed:.1f}s")
            except Exception:
                pass

        if findings:
            print_table(f"SQLi — {ip}:{port}{path}?{param}",
                        ["Tipo", "Payload", "Evidencia"], findings)
            print_result("HTTP", ip, "pwned", f"SQL INJECTION en '{param}'")
        else:
