# scripts/http/enum/banner_grab.py
...
class Script(BaseScript):
    ...
    EXAMPLES = [
        {"flag": "-t / --port", "desc": "IP y puerto HTTP",
         "good": "http --script=banner-grab -t 10.10.10.5 --port 80",
         "bad":  "http --script=banner-grab (sin -t)"},
    ]
    ...
    def run(self, **kwargs):
        ...
        print_result("HTTP", ip, "info", f"HTTP {status} — {url}")
        if rows:
        ...
        if techs:
            print_result("HTTP", ip, "info", f"Tecnologías: {', '.join(techs)}")
            session_db.save_finding(ip, "HTTP", "technologies", ", ".join(techs))

        session_db.save_finding(ip, "HTTP", "banner", f"HTTP {status} port={port}")
